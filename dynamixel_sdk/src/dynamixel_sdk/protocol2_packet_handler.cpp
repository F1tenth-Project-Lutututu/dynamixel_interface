...
unsigned short Protocol2PacketHandler::updateCRC(
  uint16_t crc_accum, uint8_t * data_blk_ptr,
...
int Protocol2PacketHandler::txRxPacket(
  PortHandler * port, uint8_t * txpacket, uint8_t * rxpacket,
  uint8_t * error)
{
  int result = COMM_TX_FAIL;
  ...
  if (txpacket[PKT_INSTRUCTION] == INST_READ) {
    port->setPacketTimeout(
      (uint16_t)(DXL_MAKEWORD(
        txpacket[PKT_PARAMETER0 + 2],
...
int Protocol2PacketHandler::ping(
  PortHandler * port, uint8_t id, uint16_t * model_number,
  uint8_t * error)
{
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::factoryReset(
  PortHandler * port, uint8_t id, uint8_t option,
  uint8_t * error)
{
...
int Protocol2PacketHandler::readRx(
  PortHandler * port, uint8_t id, uint16_t length, uint8_t * data,
  uint8_t * error)
{
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::readTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
...
int Protocol2PacketHandler::read1ByteRx(
  PortHandler * port, uint8_t id, uint8_t * data,
  uint8_t * error)
{
...
int Protocol2PacketHandler::read1ByteTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
...
int Protocol2PacketHandler::read2ByteRx(
  PortHandler * port, uint8_t id, uint16_t * data,
  uint8_t * error)
{
...
int Protocol2PacketHandler::read2ByteTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
...
int Protocol2PacketHandler::read4ByteRx(
  PortHandler * port, uint8_t id, uint32_t * data,
  uint8_t * error)
{
  ...
  if (result == COMM_SUCCESS) {
    *data =
      DXL_MAKEDWORD(
      DXL_MAKEWORD(data_read[0], data_read[1]),
      DXL_MAKEWORD(data_read[2], data_read[3]));
  }
  return result;
}
int Protocol2PacketHandler::read4ByteTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
...
int Protocol2PacketHandler::read10ByteTx(PortHandler * port, uint8_t id, uint16_t address)
{
  return readTx(port, id, address, 10); // Request to read 10 bytes
}

int Protocol2PacketHandler::read10ByteRx(
  PortHandler * port, uint8_t id, uint8_t * data,
  uint8_t * error)
{
  uint8_t data_read[10] = {0};
  int result = readRx(port, id, 10, data_read, error); // Receive 10 bytes
  if (result == COMM_SUCCESS) {
    for (int i = 0; i < 10; i++) { // Copy data to output buffer
      data[i] = data_read[i];
    }
  }
  return result;
...
int Protocol2PacketHandler::read10ByteTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
  uint8_t * data, uint8_t * error)
{
  uint8_t data_read[10] = {0};
  int result = readTxRx(port, id, address, 10, data_read, error); // Request and receive 10 bytes
  if (result == COMM_SUCCESS) {
    for (int i = 0; i < 10; i++) { // Copy data to output buffer
      data[i] = data_read[i];
    }
...
int Protocol2PacketHandler::writeTxOnly(
  PortHandler * port, uint8_t id, uint16_t address,
...
int Protocol2PacketHandler::writeTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
  uint16_t length, uint8_t * data, uint8_t * error)
{
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::write1ByteTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
  uint8_t data, uint8_t * error)
{
  uint8_t data_write[1] = {data};
...
int Protocol2PacketHandler::write2ByteTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
  uint16_t data, uint8_t * error)
{
  uint8_t data_write[2] = {DXL_LOBYTE(data), DXL_HIBYTE(data)};
...
int Protocol2PacketHandler::write4ByteTxOnly(
  ...
  uint32_t data)
{
  uint8_t data_write[4] = {DXL_LOBYTE(DXL_LOWORD(data)), DXL_HIBYTE(DXL_LOWORD(data)), DXL_LOBYTE(
      DXL_HIWORD(
        data)), DXL_HIBYTE(DXL_HIWORD(data))};
  return writeTxOnly(port, id, address, 4, data_write);
}
int Protocol2PacketHandler::write4ByteTxRx(
  PortHandler * port, uint8_t id, uint16_t address,
  uint32_t data, uint8_t * error)
{
  uint8_t data_write[4] = {DXL_LOBYTE(DXL_LOWORD(data)), DXL_HIBYTE(DXL_LOWORD(data)), DXL_LOBYTE(DXL_HIWORD(
        data)), DXL_HIBYTE(DXL_HIWORD(data))};
  return writeTxRx(port, id, address, 4, data_write, error);
}

int Protocol2PacketHandler::regWriteTxOnly(PortHandler * port, uint8_t id, uint16_t address,
  uint16_t length, uint8_t * data)
{
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::regWriteTxRx(PortHandler * port, uint8_t id, uint16_t address,
  uint16_t length, uint8_t * data, uint8_t * error)
{
  int result = COMM_TX_FAIL;
...
int Protocol2PacketHandler::syncReadTx(PortHandler * port, uint16_t start_address,
...
int Protocol2PacketHandler::syncWriteTxOnly(PortHandler * port, uint16_t start_address,
