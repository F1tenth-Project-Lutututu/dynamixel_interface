...
class WINDECLSPEC PacketHandler
{
 ...
 public:
  ...
  virtual int read10ByteTx(PortHandler *port, uint8_t id, uint16_t address) = 0;
  virtual int read10ByteRx(PortHandler *port, uint8_t id, uint8_t *data, uint8_t *error) = 0;
  virtual int read10ByteTxRx(PortHandler *port, uint8_t id, uint16_t address, uint8_t *data, uint8_t *error) = 0;
};
